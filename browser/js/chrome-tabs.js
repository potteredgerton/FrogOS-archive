...
class ChromeTabs {
  ...
  addTab(tabProperties) {
    if(tabProperties.url !== __cpLocation.protocol + "//" + __cpLocation.hostname + "/newtab.html" && !tabProperties.title) tabProperties.title = tabProperties.url;
    if(tabProperties.url !== __cpLocation.protocol + "//" +  __cpLocation.hostname +"/newtab.html") tabProperties.favicon = `https://www.google.com/s2/favicons?domain=${tabProperties.url}`;
    ...
    if(iframe.getAttribute("loading") === "true") {
      ...
      iframe.contentWindow.__cpLocation.reload();
